    ...
    public static SerialPortInfo? GetPortInfo(string portName)
    {
        using (var entity = new ManagementClass("Win32_PnPEntity"))
        {
            foreach (var instance in entity.GetInstances().Cast<ManagementObject>())
            {
                ...
                {
                    var PNPDeviceID = instance.GetPropertyValue("PnpDeviceID") as string ?? string.Empty;
                    if (!string.IsNullOrEmpty(PNPDeviceID))
                    {
                        var regPath = "HKEY_LOCAL_MACHINE\\System\\CurrentControlSet\\Enum\\" + PNPDeviceID + "\\Device Parameters";
                        if (Registry.GetValue(regPath, "PortName", "")?.ToString() == portName)
                        {
                            return new SerialPortInfo(instance);
                        }
                    }
                }
            }
